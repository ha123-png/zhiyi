...
> 当前版本：`v0.4.0 Beta`（候选版，等待本机验收后发布）。适合体验、学习和受控场景试用。所有 AI 结果在用于财务、库存或其他重要业务前，都必须人工核验。安装包见 [Releases](https://github.com/ha123-png/zhiyi/releases)。
...
- **自动名称与匹配：** 可选 AI 内容建议，或导入日期_模板_序号的固定规则；自动采用名称，保留原上传名，重试沿用名称；新建和复制模板默认不参与智能匹配，准备好后再勾选，避免重复竞争。
- **模板积累：** 字段、用途、提示、展示与规则共同版本化。可查看差异并把历史版本设为当前，不复制成冗余新版本。旧任务保留当时引用。
- **结果可核对：** 必填、范围、枚举、整段文字格式、计算与汇总规则；正则支持试填。用户决定修正或明确忽略提示。
- **原文件归档：** 桌面版选择文件后，可在提取结果保存成功时移动原文件到所选文件夹；同名、占用等问题进入待处理事项。启用即生效，不再二次确认；旧复制设置保留，内部原件仍可预览和备份。
...
- **模型方案：** 本地或云端方案、任务配置快照、连接检查、启动与加载提示、可读诊断。上下文预算可见可调，云端不继承本地 8192 加载容量；文字连接成功不等于视觉能力合格。
- **API / MCP：** 权限范围内的任务查询、限定目录导入、数据读取、带版本检查的修改与导出。默认关闭，按需授权。
- **数据仪表盘：** 当前存量与期间变化分开，支持全部时间范围、模型实际调用用量与最多四张业务统计；刷新按当前数据计算，不调用模型。图表保留数据明细、键盘操作与克制的过渡。
- **问知意：** 在指定资料范围内查找、统计、绘图与对照原件；历史对话恢复实际使用的模型；按最新数据刷新在本机复用原统计口径，不调用模型，保留旧快照并追加更新时间。修改先统一预览，再确认执行，可核对状态并撤销受支持的变更。默认只给模型三条必要明细，完整统计由后端程序计算；重复分析过程折叠，文字说明直接呈现。历史分页、按需读取，文字增量直接流式显示。模板生成和编辑统一在模板页。
...
本轮使用虚构材料实测：云端 Qwen 3.6 Flash 的文本与视觉提取、连续对话、统计绘图、修改确认及撤销链路通过。LM Studio 的 Qwen 3.5 9B 通过限定范围统计、快照复用、修改及撤销；趋势问题中模型自行添加了空值筛选，返回无数据，完整对话验收未通过。知意显示实际筛选范围，不生成虚假统计。Ollama 协议和失败路径有自动测试，本轮没有真实 Ollama 推理证据。

本轮补测 1,000 / 10,000 条含长备注的合成数据：云端 Qwen 3.6 Flash 与本地 Qwen 3.5 9B 的完整聚合、绘图及前三条读取均通过。模型请求没有塞入无关长备注，单次最大输入约 6,505 tokens。一万条云端聚合绘图约 6.4 秒、本地约 32.2 秒；耗时受模型与设备影响。连续测试另用确定性模型替身完成 200 次万条聚合、60 轮对话、停止、重启及备份恢复，未改变业务数据。
