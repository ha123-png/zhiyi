...
4. **检查结果。** 提取结果会先进入仓库；没有待处理事项时自动完成，有校验问题时保留待核对状态；开启内容命名后名称自动生效，无需单独采纳。入表不等于已经人工检查。
...
- **自动名称与匹配：** 开启内容命名后自动采用有效名称，保留原上传名；新建和复制模板默认不参与智能匹配，准备好后再勾选，避免重复竞争。
...
- **问知意：** 在指定资料范围内查找、统计、绘图与对照原件；复用分析快照，修改先统一预览，再确认执行，可核对状态并撤销受支持的变更。模板生成和编辑统一在模板页。
