...
const questions: { title: string; text: string; target: NavigationKey; action: string }[] = [
  ...
  { title: "原件、内容名称和导出副本是什么关系？", text: "内部原件由知意保存，用于预览和追溯。开启“有意义的文件名”后，名称随提取结果自动生效，无需单独采纳，仍可编辑；有意义的原名可保留。外部副本也是可选项，复制完整文件到指定位置；不会覆盖同名文件，也不会自动改名、移动或删除已经导出的副本。完整原件在文件历史下载。", target: "history", action: "查看文件历史" },
