...
const questions: { title: string; text: string; target: NavigationKey; action: string }[] = [
  { title: "表有很多数据，问知意会全部读取吗？", text: "不会把整张表塞进对话。默认提供最多三条明细或分组预览，长字段控制长度；需要时再按字段、筛选和排序读取。统计覆盖完整授权范围，图表使用完整分析快照。重复处理过程默认收起，文字说明直接显示。对话历史分页加载，可搜索更早的对话；停止、短暂断线或重启后，已保存内容仍可继续查看。", target: "assistant", action: "打开问知意" },
  { title: "问知意能帮我做什么？会把数据发到哪里？", text: "可以直接提问，也可以从仓库、任务或模板页面打开侧栏，附加当前范围，让它查询、分析、画图，或解释模板的字段与规则。问知意单独选择模型，不沿用提取方案。使用远程模型前需授权发送问题、历史及所选资料；同一对话、方案与范围内无需逐句确认，可在对话设置中撤销。默认先预览修改，确认后执行；明确选择“本次委托”才会在下一次发送中自动执行授权内的部分修改。模板的创建、修改和版本恢复统一在模板页完成。打开历史对话会恢复当时使用的模型与资料范围，设置显示下一条消息使用的模型。图表保留生成时的数值与来源；按最新数据重新分析会在本机复用原统计口径，追加新结果和更新时间，不调用模型。", target: "assistant", action: "打开问知意" },
